 ...
 * You control the slot's shape; it sizes to its container by default. The
 * page author can prefill the slot by writing a photo URL into src (with
 * credit/credit-href); the user can still fill or replace it by dragging
 * an image file onto it (or clicking to browse). The dropped image
 * persists across reloads via a .image-slots.state.json sidecar (read via
 * fetch, written via window.omelette), so the filled slot shows on share
 * links, downloaded zips, and PPTX export. Outside the omelette runtime
 * the slot is read-only.
 *
 * The sidecar is a SIBLING of the HTML file that uses this component: the
 * read is a document-relative fetch, and the host resolves the bridge's
 * sidecar writes into the previewed file's directory to match. Pages in
 ...
 *   src          Optional initial/fallback image URL. Prefill it with a
 *                real photo URL and set credit/credit-href alongside.
 *                A user drop overrides it; clearing the drop reveals src
 *                again.
...
(() => {
  ...
  const UNSPLASH_HOMEPAGE_HREF =
    'https://unsplash.com/?utm_source=proteus_visual_language&utm_medium=referral';
  // Host rule: apex or any subdomain of unsplash.com. Unsplash+ results
  // serve from plus.unsplash.com, not just images.*, and an
  // admitted-but-uncredited photo must error whatever unsplash host it
  // rides on.
  ...
  const withReferral = (href) => {
    try {
      ...
      if (!u.searchParams.has('utm_source')) {
        u.searchParams.set('utm_source', 'proteus_visual_language');
      }
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // Content. The sidecar can be written outside this component, so its
      // value isn't guaranteed canvas-originated; only accept data:image/
      // URLs from it. The `src` attribute is author-controlled, so it
